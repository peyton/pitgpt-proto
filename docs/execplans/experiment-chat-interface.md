...
The app surface is the Tauri native wrapper in `app`. Because Tauri renders the same React frontend, native compatibility mostly means preserving the state schema through native storage and routing native ingestion through the same chat abstraction. Where the HTTP API can stream progress, native ingestion will emit equivalent client-side trace messages around the existing `ingest_local` command.
...
- Observation: The existing app has no server-side experiment persistence; all durable product state is client-side `localStorage` in web and `pitgpt_state.json` in Tauri.
  Evidence: `web/src/lib/storage.ts` stores `AppState` and `app/src/storage.rs` only saves and loads a JSON value.
...
The ingestion pipeline turns a user query and optional documents into an `IngestionResult`. The Python implementation is in `src/pitgpt/core/ingestion.py`, and the FastAPI endpoint for web mode is `src/pitgpt/api/main.py` at `POST /ingest`. The Tauri local path is `web/src/lib/api.ts` calling native command `ingest_local`, implemented in `app/src/commands.rs` and `app/src/ingestion.rs`.
...
Durable state is defined in `web/src/lib/types.ts` and normalized in `web/src/lib/storage.ts`. Current storage version is `4` after this change. The native app saves that same JSON through `save_app_state` in `app/src/commands.rs`. Therefore schema migration belongs in `web/src/lib/storage.ts`, not in Rust.
